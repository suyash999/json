"""
data_manager.py — Clean persistence layer for eBay IAC Artifact Vault.
...
ARTIFACT_TYPES = {
    "zeta_link":   {"icon": "🔗", "label": "Zeta",       "color": "#0064D2"},
    "github_link": {"icon": "🐙", "label": "GitHub",     "color": "#24292e"},
    "tableau_link":{"icon": "📊", "label": "Tableau",    "color": "#E97627"},
    "generic_link":{"icon": "🌐", "label": "Link",       "color": "#6366f1"},
    "powerpoint":  {"icon": "📑", "label": "PowerPoint", "color": "#D04423"},
    "excel":       {"icon": "📗", "label": "Excel",      "color": "#217346"},
    "pdf":         {"icon": "📕", "label": "PDF",        "color": "#E53238"},
    "other_file":  {"icon": "📎", "label": "File",       "color": "#6B7280"},
}
...
def save_data(data: dict, skip_sync: bool = False):
    _ensure_dirs()
    data["last_updated"] = _now()
    ...
    # 3. Sync to Google Sheets (non-blocking — failures don't break the app)
    if not skip_sync:
        try:
...
def load_from_google_sheet() -> tuple[dict | None, str]:
...
def rename_team(old: str, new: str) -> tuple[bool, str]:
    data = load_data()
    new = new.strip()
    if not new:
...
def delete_team(name: str, pw: str) -> tuple[bool, str]:
...
def rename_member(team: str, old: str, new: str) -> tuple[bool, str]:
    data = load_data()
    new = new.strip()
    if not new:
        return False, "Name cannot be empty."
...
def delete_member(team: str, name: str, pw: str) -> tuple[bool, str]:
    if pw != ADMIN_DELETE_PASSWORD:
        return False, "Wrong password."
    data = load_data()
    if team not in data["teams"]:
...
def rename_artifact(team: str, member: str, aid: str, new: str) -> tuple[bool, str]:
    data = load_data()
    new = new.strip()
    if not new:
        return False, "Name cannot be empty."
...
def delete_artifact(team: str, member: str, aid: str, pw: str) -> tuple[bool, str]:
    if pw != ADMIN_DELETE_PASSWORD:
        return False, "Wrong password."
    data = load_data()
...
def classify_link(url: str) -> str:
    u = url.lower()
    if "github" in u: return "github_link"
    if "tableau" in u: return "tableau_link"
    if "zeta" in u: return "zeta_link"
    return "generic_link"
