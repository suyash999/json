...
3. Replace credentials/service_account.json with your real key
4. Create a Google Sheet and share it with the service account email
   (the client_email from the JSON key) with Editor access
5. Set GOOGLE_SHEET_ID in config.py to your sheet's ID
...
# Import config — sheet ID and toggle
try:
...
def _is_placeholder_credentials() -> bool:
    """Check if the credentials file still has placeholder values."""
...
def _get_client():
    ...
    if not GOOGLE_SYNC_ENABLED:
        logger.info("Google Sync disabled in config.")
    ...
    try:
        ...
        logger.warning("gspread or google-auth not installed — sync disabled. "
...
def is_sync_available() -> bool:
    """Check if Google Sheets sync is available and configured."""
    _get_client()
    return _gsheet_available is True


def get_sync_status() -> dict:
    """Return sync status for display in the UI."""
    if not GOOGLE_SYNC_ENABLED:
        return {"status": "disabled", "message": "Google Sync disabled in config.py", "icon": "⚪"}

    if _is_placeholder_credentials():
        return {"status": "unconfigured", "message": "Credentials are placeholder — replace with real service account key", "icon": "🟡"}
...
def sync_to_sheet(data: dict) -> tuple[bool, str]:
    """
    Write the full vault data to Google Sheets.

    Sheet layout:
      - Sheet 1 ("Vault JSON"): Full JSON backup in cell A1
      - Sheet 2 ("Flat View"):  Flattened table: Team | Member | Artifact | Type | URL | Date
      - Sheet 3 ("Sync Log"):   Timestamp log of every sync
...
def load_from_sheet() -> tuple[Optional[dict], str]:
    """
    Load vault data from Google Sheet (Sheet 1 = full JSON backup).
    Returns (data_dict | None, message)
    """
    client = _get_client()
    if client is None:
...
def _get_or_create_sheet(spreadsheet, title: str, index: int):
